...
retry:
	...
	list_add_tail(&s->s_list, &super_blocks);//super_block插入到全局链表中
	list_add(&s->s_instances, &type->fs_supers);//super_block插入到file_system_file对象中
	spin_unlock(&sb_lock);
	get_filesystem(type);
	return s;
}
...
vfs_kern_mount(struct file_system_type *type, int flags, const char *name, void *data)
{
	...
	error = -ENOMEM;
	mnt = alloc_vfsmnt(name);//从内存中创建vfsmount对象
	if (!mnt)
		goto out;
	...
	error = type->get_sb(type, flags, name, data, mnt);//创建super block 、dentry、inode
	if (error < 0)
		goto out_free_secdata;
...
{
	struct file_system_type *type = get_fs_type(fstype);//此处fstype实际为文件系统名字 例如ext3
	struct vfsmount *mnt;
	if (!type)
		return ERR_PTR(-ENODEV);
